...
sync_one() {
    ...
    cp "${src}" "${dst}"
    echo "[chezmoi] updated ${name}: ${src} -> ${dst}"
}

# --- Native Linux sync ---
sync_one "vscode-linux" "${SRC_VSCODE}" \
    "${HOME}/.config/Code/User/keybindings.json" \
    "${HOME}/.config/Code"

sync_one "cursor-linux" "${SRC_CURSOR}" \
